...
export class QuizHeaderComponent implements OnInit {
  ...
  onSubmit(quizForm: NgForm) {
    ...
    this.triviaService.getQuestions(category, difficulty)
      .subscribe(triviaQuestions => {
        const questions: Question[] = triviaQuestions.map(triviaQuestion => this.parseTriviaQuestion(triviaQuestion));
